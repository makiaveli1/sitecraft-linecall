...
The permanent HTTPS deployment is [grassy-lotus-7dr8.here.now](https://grassy-lotus-7dr8.here.now/). Across attended checks on 2026-08-31 and 2026-09-01, ChatGPT's WebMCP-capable in-app browser discovered the four normal-session tools on that exact origin, successfully ran the snapshot, strategy-comparison, and exact-preview path, and confirmed that human-locked cue Q014 blocks both Opening retime strategies without producing a plan. On 2026-09-01, the same permanent origin completed the attended native approval lifecycle: four tools at R1, exact human approval of `retime-r1-qa-p2-ripple_after`, five tools including the temporary apply capability, one exact WebMCP apply, then R2 with a visible 13-cue receipt and four tools again. The final R2 snapshot had no active approval or conflicts, with Q020 at `00:24` and Q032 at `00:47`.
